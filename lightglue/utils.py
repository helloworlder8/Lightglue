...
def numpy_image_to_torch(image: np.ndarray) -> torch.Tensor:
    ...
    if image.ndim == 3:
        image = image.transpose((2, 0, 1))  # HxWxC to CxHxW 转换成通道宽高的格式
    elif image.ndim == 2:
        image = image[None]  # add channel axis
    else:
        raise ValueError(f"Not an image: {image.shape}")
    return torch.tensor(image / 255.0, dtype=torch.float) #归一化
...
def load_image(path: Path, resize: int = None, **kwargs) -> torch.Tensor:
    image = read_image(path) #以RGB格式读取图片
...
class Extractor(torch.nn.Module):
    ...
    def extract(self, img: torch.Tensor, **conf) -> dict:
        ...
        if img.dim() == 3:
            img = img[None]  # add batch dim 增加批次 torch.Size([1, 3, 682, 1024])
        assert img.dim() == 4 and img.shape[0] == 1
        shape = img.shape[-2:][::-1] #-》torch.Size([1024, 682]) 宽 高
