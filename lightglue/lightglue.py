...
class LearnableFourierPositionalEncoding(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor: #torch.Size([1, 2048, 2])
...
class SelfBlock(nn.Module):
    ...
    def forward(
        ...
        qkv = self.Wqkv(x) #torch.Size([1, 2048, 256]) -》torch.Size([1, 2048, 768])
        qkv = qkv.unflatten(-1, (self.num_heads, -1, 3)).transpose(1, 2)
        q, k, v = qkv[..., 0], qkv[..., 1], qkv[..., 2] #-》torch.Size([1, 4, 2048, 64]) 批 头 点 维
        q = apply_cached_rotary_emb(encoding, q) #加入位置编码
...
class TransformerLayer(nn.Module):
    ...
    def forward(
        ...
        if mask0 is not None and mask1 is not None:
            ...
            desc0 = self.self_attn(desc0, encoding0) #-》torch.Size([1, 2048, 256])
            desc1 = self.self_attn(desc1, encoding1) #-》torch.Size([1, 2048, 256])
...
class LightGlue(nn.Module):
    ...
    def _forward(self, data: dict) -> dict:
        ...
        data0, data1 = data["image0"], data["image1"] #每张图片经过提取器后的所有信息
        kpts0, kpts1 = data0["keypoints"], data1["keypoints"] #关键点坐标
        b, m, _ = kpts0.shape #1 2048，2
        b, n, _ = kpts1.shape
        device = kpts0.device
        ...
        encoding0 = self.posenc(kpts0) #torch.Size([2, 1, 1, 2048, 64])
        encoding1 = self.posenc(kpts1) #torch.Size([2, 1, 1, 2048, 64])
